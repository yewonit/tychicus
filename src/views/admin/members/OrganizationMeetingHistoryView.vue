    ...
    computed: {
      ...
      meetingSummary() {
        ...
        this.meetings.forEach((meeting) => {
          ...
          groupedMeetings[activityName].totalAbsent +=
    ...
    methods: {
        ...
        try {
          ...
          for (const leafOrg of leafOrgs) {
            try {
              ...
              console.error(
                `조직 ID ${leafOrg.id}의 멤버 수 조회 중 오류:`,
                err
              );
              leafOrg.memberCount = 0;
            }
      ...
      formatAttendances(attendances) {
        ...
        const result = attendances.map((attendance) => {
          ...
          return {
            ...
            roleName:
              attendance.roleName || attendance.role_name || '일반 회원',
          };
        });
      ...
      async exportToExcel() {
        try {
          ...
          // 데이터 추가
          this.meetings.forEach((meeting) => {
            const meetingDate = moment(meeting.meeting_date).format(
              'YYYY-MM-DD'
            );
